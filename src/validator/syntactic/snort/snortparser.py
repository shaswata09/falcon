...
def snort_syntax_checker(snort_rule: str) -> dict:
    result = {"status": True, "error_comment": "No syntax errors detected."}

    try:
        # Attempt to parse the Snort rule from the string
        flag = basic_snort_rule_parser(snort_rule)
        if not flag["status"]:
            parser = Parser(snort_rule)
            if not parser.is_valid:
                raise ValueError("Invalid Snort rule syntax.")
...
def basic_snort_rule_parser(snort_rule: str) -> dict:
    result = {"status": True, "error_comment": "No syntax errors detected."}
    ...
    match = rule_pattern.match(snort_rule)
    if not match:
        result["status"] = False
        result["error_comment"] = "Rule does not match basic Snort rule structure."
        return result

    # Check option block is well-formed
    options_block = match.group(8)
    if not options_block:
        result["status"] = False
        result["error_comment"] = "Missing or empty option block."
        return result

    # Split options on ';' and check for key-value format where appropriate
    ...
    for opt in options:
        if ":" in opt:
            ...
            if not key.strip() or not value.strip():
                result["status"] = False
                result["error_comment"] = f"Malformed option: '{opt}'"
                return result
        else:
            # Some options don't require a value (e.g., 'nocase')
            if not re.match(r"^\w+$", opt):
                result["status"] = False
                result["error_comment"] = (
                    f"Unrecognized or malformed standalone option: '{opt}'"
                )
                return result
...
if __name__ == "__main__":
    rule = (
        'alert tcp $HOME_NET any -> $EXTERNAL_NET 80 (msg:"Example rule"; sid:1000001;)'
    )
